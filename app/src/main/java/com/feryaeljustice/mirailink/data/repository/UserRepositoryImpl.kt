...
class UserRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun deleteUserPhoto(position: Int): MiraiLinkResult<Unit> = remote.deleteUserPhoto(position)
        ...
        override suspend fun getUserById(userId: String): MiraiLinkResult<User> =
            when (val result = remote.getUserById(userId)) {
                is MiraiLinkResult.Success -> {
                    val (userDto, photos) = result.data
                    val domainUserPhotos = photos.map { it.toDomain() }
                    val orderedPhotos = resolvePhotoUrls(baseUrl, domainUserPhotos)
        ...
        override suspend fun uploadUserPhoto(photo: Uri): MiraiLinkResult<String> = remote.uploadUserPhoto(photo)

        override suspend fun saveUserFCM(fcm: String): MiraiLinkResult<Unit> = remote.saveUserFCM(fcm = fcm)
    }
