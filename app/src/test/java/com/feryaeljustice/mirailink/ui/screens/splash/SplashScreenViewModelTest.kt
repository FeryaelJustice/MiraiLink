...
class SplashScreenViewModelTest {
    ...
    @Test
    fun `when onboarding is not completed, navigate to onboarding`() = runTest {
        coEvery { checkAppVersionUseCase.invoke(any()) } returns MiraiLinkResult.Success(VersionCheckResult(mustUpdate = false, shouldUpdate = false, "", ""))
        coEvery { checkOnboardingIsCompletedUseCase.get().invoke() } returns MiraiLinkResult.Success(false)
        coEvery { autologinUseCase.get().invoke() } returns MiraiLinkResult.Error("")

        viewModel = SplashScreenViewModel(checkAppVersionUseCase, autologinUseCase, checkOnboardingIsCompletedUseCase)
    ...
    @Test
    fun `when onboarding is completed and autologin succeeds, navigate to home`() = runTest {
        coEvery { checkAppVersionUseCase.invoke(any()) } returns MiraiLinkResult.Success(VersionCheckResult(mustUpdate = false, shouldUpdate = false, "", ""))
        coEvery { checkOnboardingIsCompletedUseCase.get().invoke() } returns MiraiLinkResult.Success(true)
    ...
    @Test
    fun `when onboarding is completed and autologin fails, navigate to auth`() = runTest {
        coEvery { checkAppVersionUseCase.invoke(any()) } returns MiraiLinkResult.Success(VersionCheckResult(mustUpdate = false, shouldUpdate = false, "", ""))
        coEvery { checkOnboardingIsCompletedUseCase.get().invoke() } returns MiraiLinkResult.Success(true)
        coEvery { autologinUseCase.get().invoke() } returns MiraiLinkResult.Error("")
    ...
    @Test
    fun `when must update, show update dialog`() = runTest {
        coEvery { checkAppVersionUseCase.invoke(any()) } returns MiraiLinkResult.Success(VersionCheckResult(mustUpdate = true, shouldUpdate = false, "", ""))
        coEvery { checkOnboardingIsCompletedUseCase.get().invoke() } returns MiraiLinkResult.Success(true)
        coEvery { autologinUseCase.get().invoke() } returns MiraiLinkResult.Success("token")
