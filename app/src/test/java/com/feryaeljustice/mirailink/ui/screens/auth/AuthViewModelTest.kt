...
class AuthViewModelTest {
    ...
    fun `login success without 2fa`() =
        runTest {
            ...
            coEvery { loginUseCase.get().invoke(email, "", password) } returns
                MiraiLinkResult.Success(
                    token,
                )
            coEvery { getTwoFactorStatusUseCase.get().invoke(userId) } returns
                MiraiLinkResult.Success(
                    false,
                )
    ...
    fun `register success`() =
        runTest {
            ...
            every { JwtUtils.extractUserId(token) } returns userId
            coEvery {
                registerUseCase.get().invoke(username, email, password)
