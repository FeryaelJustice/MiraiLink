...
class FeedbackViewModelTest {
    ...
    @Before
    fun setUp() {
        viewModel = FeedbackViewModel(sendFeedbackUseCase)
    }
    ...
    fun `send feedback success`() =
        runTest {
            ...
            coEvery { sendFeedbackUseCase.get().invoke(feedback) } returns MiraiLinkResult.Success(Unit)
    ...
    fun `send feedback error`() =
        runTest {
            ...
            coEvery { sendFeedbackUseCase.get().invoke(feedback) } returns MiraiLinkResult.Error(errorMessage)
