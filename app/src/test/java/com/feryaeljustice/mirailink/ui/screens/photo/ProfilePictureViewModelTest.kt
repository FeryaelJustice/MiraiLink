...
class ProfilePictureViewModelTest {
    ...
    @Before
    fun setUp() {
        viewModel = ProfilePictureViewModel(uploadUserPhotoUseCase, mainCoroutineRule.testDispatcher)
    }
    ...
    fun `upload image success`() =
        runTest {
            ...
            coEvery { uploadUserPhotoUseCase.get().invoke(uri) } returns MiraiLinkResult.Success(imageUrl)
    ...
    fun `upload image error`() =
        runTest {
            ...
            coEvery { uploadUserPhotoUseCase.get().invoke(uri) } returns MiraiLinkResult.Error(errorMessage)
