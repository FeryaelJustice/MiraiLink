...
class MessagesViewModelTest {
    ...
    private val chatSummary = ChatSummary("1", ChatType.PRIVATE, "", Date(), Date(), ChatRole.MEMBER)

    @Before
    fun setUp() {
        val getChatsFromUser = mockk<GetChatsFromUser>()
        ...
        viewModel = MessagesViewModel(getMatchesUseCase, chatUseCases)
    ...
    fun `load data success`() =
        runTest {
            ...
            assert((state as MessagesViewModel.MessagesUiState.Success).openChats.isNotEmpty())
        }
    ...
    fun `load chats error`() =
        runTest {
            val getChatsFromUser = mockk<GetChatsFromUser>()
            coEvery { chatUseCases.get().getChatsFromUser } returns getChatsFromUser
