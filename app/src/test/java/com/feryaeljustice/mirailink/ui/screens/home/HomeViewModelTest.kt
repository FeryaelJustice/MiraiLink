...
class HomeViewModelTest {
    ...
    private val user1 = User("1", "user1", "user1", "user1@test.com", null, null, null, null, emptyList(), emptyList(), emptyList())
    private val user2 = User("2", "user2", "user2", "user2@test.com", null, null, null, null, emptyList(), emptyList(), emptyList())

    @Before
    fun setUp() {
        coEvery { getCurrentUserUseCase.get().invoke() } returns MiraiLinkResult.Success(user1)
        coEvery { getFeedUseCase.get().invoke() } returns MiraiLinkResult.Success(listOf(user1, user2))
