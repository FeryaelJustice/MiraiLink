...
class VerificationViewModelTest {
    ...
    @Before
    fun setUp() {
        viewModel = VerificationViewModel(checkIsVerifiedUseCase, requestVerificationCodeUseCase, confirmVerificationCodeUseCase, mainCoroutineRule.testDispatcher)
    }
    ...
    fun `request code success`() =
        runTest {
            val userId = "userId"
            coEvery { requestVerificationCodeUseCase.get().invoke(userId, "email") } returns MiraiLinkResult.Success("Success")
    ...
    fun `confirm code success`() =
        runTest {
            val userId = "userId"
            val token = "token"
            coEvery { confirmVerificationCodeUseCase.get().invoke(userId, token, "email") } returns MiraiLinkResult.Success("Success")
    ...
    fun `confirm code error`() =
        runTest {
            ...
            coEvery { confirmVerificationCodeUseCase.get().invoke(userId, token, "email") } returns MiraiLinkResult.Error(errorMessage)
