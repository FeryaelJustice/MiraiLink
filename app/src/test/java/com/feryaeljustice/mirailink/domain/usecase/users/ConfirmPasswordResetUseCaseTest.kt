...
class ConfirmPasswordResetUseCaseTest {
    ...
    @Before
    fun onBefore() {
        confirmPasswordResetUseCase = ConfirmPasswordResetUseCase(repo)
    }
    ...
    fun `when repository confirms password reset successfully, return success`() = runTest {
        ...
        // When
        val result = confirmPasswordResetUseCase("test@example.com", "token", "newPassword")
    ...
    fun `when repository throws an exception, return error`() = runTest {
        ...
        coEvery { repo.confirmPasswordReset(any(), any(), any()) } throws exception
