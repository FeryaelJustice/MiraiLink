...
class DisableTwoFactorUseCaseTest {
    ...
    @Before
    fun onBefore() {
        disableTwoFactorUseCase = DisableTwoFactorUseCase(repository)
    }

    @Test
    fun `when repository disables 2FA successfully, return success`() = runTest {
        // Given
        val code = "123456"
        coEvery { repository.disable2FA(code) } returns MiraiLinkResult.Success(Unit)

        // When
        val result = disableTwoFactorUseCase(code)
    ...
    @Test
    fun `when repository fails to disable 2FA, return error`() = runTest {
        // Given
        val code = "123456"
        val errorResult = MiraiLinkResult.Error("Invalid code")
        coEvery { repository.disable2FA(code) } returns errorResult
    ...
    fun `when repository throws an exception, return error`() = runTest {
        ...
        coEvery { repository.disable2FA(code) } throws exception
